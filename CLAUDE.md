...
> **版本**: v25.2 (Phase 25.2 — 自治并发工作流 adoption: 双会话自推进 + 自服务 [collaboration/BACKLOG.md](collaboration/BACKLOG.md) + **全套门绿后自 ff-merge 到 master**(非受影响子集)，常驻 worktree `track-a`(前端)/`track-b`(后端)。协调者仅裁决真正需人肉的事: 跨轨道抢同一契约文件 / 门始终不绿 / 破坏架构不变量。文档: [COORDINATION.md](COORDINATION.md) §3 新增自治契约、§2 改用常驻 worktree 路径、§6 shim 归前端 Track A 自理; BACKLOG.md 后端 B 自服务队列 P2-QUEUE→RESTART→MULTI; CURRENT_STATUS.md 对齐 v25.2。**无源码变更**——主要成因为消除"逐任务人工派发/合并"的串行瓶颈，让两会话互不空等。待办(自服务): 前端 P1-SHIM + P2 预览抽屉/DTO 迁移/Insight 集成; 后端 P2-QUEUE→RESTART→MULTI)
