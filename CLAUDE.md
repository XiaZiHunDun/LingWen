...
> **版本**: v25.4 (Phase 25.4 — 收尾完善：类型债清零 + batch templates 前端闭环 + 依赖修复 + 仓库清理。类型债: vue-tsc typecheck:app 5 pre-existing + tests-relaxed 3 pre-existing 全部清零（useStudioProject JSDoc 返回类型 + usePilotBatch.runPreflight 参数类型 + 3 处 creator DTO cast 改用具体契约类型 CreatorMemoryQueryResult/CreatorPublishEntry/CreatorVolumeTemplateApproval + pilot-history-list spec 补 slug）。功能闭环: batch templates 前端消费 — PilotTemplatePanel.vue（选择/应用/保存/删除）+ api/studio.ts list/create/deleteStudioBatchTemplate + usePilotBatch templates 状态/loadTemplates/saveTemplate/deleteTemplate + PilotStartForm defineExpose currentForm/fillFromTemplate + PilotPage 接线 + 6 新 vitest。依赖: pyproject 增 socksio>=1.0 + uv.lock（httpx SOCKS 代理访问 LLM provider 缺失修复，lingwen-llm 11 passed）。仓库: 清理 stash@{0} trackb-baseline-check-residual（确认无价值）。验证: 前端全量 vitest 1850 passed + 1 skipped / ESLint 0 / knip issues 0 / vue-tsc 0 error；后端 lingwen-llm 11 passed + studio_api/lingwen-shared 221 passed（+1 环境基线 `_IncludedRouter`，starlette 1.6 回归，master 基线一致）+ ruff clean。遗留: tests/ci/ 94 个迁移路径陈旧失败为环境基线（dashboard/frontend 旧路径 + lingwen_creator 未装包），与本次改动无关）
 → v25.3 (Phase 25.3 — 自治双会话合流: Track A(前端) REQ-001 创作者模式增强 5 切片 + P2-QUEUE 前端集成 + Track B(后端) P2-QUEUE/RESTART/MULTI 全部入 master，master `3d009514`。后端(Track B 自服务已合): P2-QUEUE batch priority queue with auto-promotion (BatchJob `queued` 状态 + priority/skip_preflight + `submit_batch_job` 忙则排队 + `advance_batch_queue` 自动晋升保持 job_id 连续性 + `GET /api/studio/batch/queue` + `POST /api/studio/production/run` 可选 `priority` 参数) + P2-RESTART auto-restart on failure (max_attempts 上限 + 同 job_id 重试，默认 1 零行为变化) + P2-MULTI multi-LLM concurrent batch with per-chapter provider。前端(Track A，本次整合): REQ-001 A/B `useCreatorMode` 单一来源 + 陪伴「今日下一步」卡 / C 推进「批改节奏带」`CreatorBatchRhythm.vue`(只读复用 usePilotBatch + 越序偏差提示) / D 写栏「模式引导条」`CreatorModeGuideBar.vue`(可关闭+按模式本地记忆) / E 「差异收尾」`CreatorDeviationFinalize.vue` + `utils/batchDeviation.ts`(复核状态按批次持久化) + P2-QUEUE 前端集成 `PilotQueuePanel.vue` + `listStudioBatchQueue`(GET /studio/batch/queue?slug=, 与后端 `queued` 状态契约对齐)。验证(整合态协调者复核): 前端全量 vitest 1844 passed + 1 skipped / ESLint 0 / knip issues 0 / vue-tsc typecheck:app 5 pre-existing + tests-relaxed 3 pre-existing 均逐条与 master 基线一致 **ZERO new**。协调点: CURRENT_STATUS.md merge 冲突解决(保留 track-a REQ-001 记录 + 采纳 master P2-MULTI 完成态); 主仓发现 Track B 基线对比残留(stash@{0} 保留) + track-b worktree `projects/anye-xinbiao/docs/novel-pillars.md` 垃圾残留(丢弃) + 游离 worktree `/tmp/trackb_baseline` 清理)
  → v25.2 (Phase 25.2 — 自治并发工作流 adoption: 双会话自推进 + 自服务 [collaboration/BACKLOG.md](collaboration/BACKLOG.md) + **全套门绿后自 ff-merge 到 master**(非受影响子集)，常驻 worktree `track-a`(前端)/`track-b`(后端)。协调者仅裁决真正需人肉的事: 跨轨道抢同一契约文件 / 门始终不绿 / 破坏架构不变量。文档: [COORDINATION.md](COORDINATION.md) §3 新增自治契约、§2 改用常驻 worktree 路径、§6 shim 归前端 Track A 自理; BACKLOG.md 后端 B 自服务队列 P2-QUEUE→RESTART→MULTI; CURRENT_STATUS.md 对齐 v25.2。**无源码变更**——主要成因为消除"逐任务人工派发/合并"的串行瓶颈，让两会话互不空等。待办(自服务): 前端 P1-SHIM + P2 预览抽屉/DTO 迁移/Insight 集成; 后端 P2-QUEUE→RESTART→MULTI)
  → v25.1 (Phase 25.1 — 双会话并行开发合流 — Track A(前端) event_types 过滤开关 + Track B(后端) batch templates，由协调者在 master 串行 ff-merge 至 `1c1d3a82`，0 冲突；新增 `COORDINATION.md` 作为并行开发唯一入口。后端: `infra/studio_batch_templates.py` (BatchTemplate 数据类 + JSON 文件持久化 `infra/.state/studio_batch_templates/`) + `POST/GET/GET{id}/PUT{id}/DELETE{id} /api/studio/batch/templates` (event 偏好按 KNOWN_EVENT_TYPES 校验 400/404/400 映射) + lingwen_shared DTO (StudioBatchTemplate/(Create|Update)Request/ListResponse) codegen 出 ts/studio.ts。前端: PilotLivePanel 轻量 event-type toggle-chips 过滤 UI → usePilotBatch.selectedEventTypes → useBatchEventStream `{eventTypes}` 经 query `?event_types=` 传 SSE 流 (filter 变化即重连重建参数)。关键教训(合并门复核): Track A 自报 vue-tsc ZERO new 不实 — 实为新增 9 个测试文件类型错误(pilot-live-panel fixture `status: string` 放宽 + use-batch-event-stream 2 处错误 cast)，协调者修复后回到 5 个 pre-existing PilotPage debt (`1c1d3a82`)；Track B 的 lingwen_shared 是 editable-install(pth 指向主仓)，worktree 跑测试/codegen 需前置 PYTHONPATH=worktree 副本否则导入主仓旧实现(合并态主仓一致即 32 passed)。验证(合并态): 后端 pytest 32 passed + ruff clean + ruff format --check clean；前端 vitest 受影响 spec 通过 + ESLint 0 + knip `{"issues":[]}` + vue-tsc 5 pre-existing ZERO new. 待办 Phase 26+: 前端需补 batch-templates 的 dashboard-contracts *DTO shim(后端只 codegen lingwen_shared 源头、未 touch 前端契约，新 DTO 加法不破坏编译但前端暂消费不到新类型) + per-chapter preview drawer + multi-LLM concurrent batches + usePilotBatch DTO migration. See `COORDINATION.md` §5)
  → v25.0 (Phase 25 — SSE Batch 增强 Filter/Replay/Auth — 8 source commits on branch `phase-25-sse-batch-enhance`; enhances Phase 24 SSE. Backend: `infra/studio_batch_streamer.py` per-subscriber `event_types` filter (`_Subscriber(queue, event_types)`; `KNOWN_EVENT_TYPES`; `publish` skips non-matching subscribers; `unsubscribe` matches by queue identity preserving Phase 24) + `infra/studio_batch_runner.py` new `replay_events(job)` (reconstructs `job_state` + `chapter_completed` from on-disk chapters + terminal event) + `GET /api/studio/batch/<job_id>/events` gains query params — `event_types` (validation → 400), `replay=1` (emits deterministic disk history on connect), `slug`/`mode` guards (mismatch → 403), reads gated by `LINGWEN_ALLOW_DASHBOARD_BATCH` (403 when disabled); initial events built once + filtered, preserving Phase 24 terminal-close. Frontend: `useBatchEventStream(jobId, options)` with `replay`/`eventTypes` → `buildUrl()` query params; `usePilotBatch` passes `{replay:true}` so `chapterEvents` repopulates on connect. 1 NEW architecture invariant (#50 SSE 事件流支持服务端 event_types 过滤 + 连接时磁盘 replay; 读取被 LINGWEN_ALLOW_DASHBOARD_BATCH 门控). Backend: 2 streamer filter tests + 6 route tests (replay/guard/400/403); affected subsets 440 pass + 5 documented baseline env fails (LLM Provider/_IncludedRouter/socksio — ZERO new, verified identical on master); ruff clean + ruff format --check clean. Frontend: vitest 1810 + 1 skipped (+3 NEW) / ESLint 0 / knip `{"issues":[]}` / vue-tsc only 5 pre-existing PilotPage phase-23 debt (baseline, ZERO new). key facts: knip flags exported-but-only-internal types (keep `BatchEventType` module-private; drop it from composables/index.ts barrel to avoid vue-tsc TS2724) + FastAPI `Query()` function-args for SSE route query params + disk replay only covers `job_state`+`chapter_completed` (`chapter_started` not reliably reconstructable) + blocking TestClient hangs on running-job SSE streams (use terminal job in tests) + full 11min O(6000)-test suite skipped after non-zero baseline (documented env failures unrelated to phase). Carryover to Phase 26+: per-chapter preview drawer in PilotLivePanel + batch templates + multi-LLM concurrent batches + Pilot + Insight dashboard integration + `usePilotBatch` DTO migration + event_types optional UI filter toggle. See `docs/superpowers/handoffs/2026-09-02-phase-25-sse-batch-enhance-handoff.md`)
  → v24.0 (Phase 24 — SSE Real-time Batch Progress feature phase — 8 source commits on branch `phase-24-sse-batch-progress`; replaces Phase 23 3s scraping polling with server-push real-time progress (closes Phase 23 carryover #1). Backend: NEW `infra/studio_batch_streamer.py` (BatchEventPublisher module-level thread-safe singleton with per-job subscriber list + asyncio.Queue cap 100 drop-oldest + BatchEventStreamer) + `GET /api/studio/batch/<job_id>/events` SSE route (StreamingResponse + Cache-Control no-cache + X-Accel-Buffering no + 15s heartbeat + non-streaming 404/401/403) + 6 named events (job_state/chapter_started/chapter_completed/job_completed/job_failed/job_cancelled) + 4 hooks in `infra/studio_batch_runner.py`. Frontend: NEW `useBatchEventStream` composable (native EventSource, 50-event FIFO buffer, addEventListener per named event) + `usePilotBatch` drops 3s polling for SSE (processedEventIndex consume-each-once + 5s fallback polling ONLY when SSE disconnected) + `chapterEvents` ref + `PilotLivePanel` chapterEvents prop renders recent 5. 1 NEW architecture invariant (#49 SSE推 batch 状态为 canonical; 3s polling dropped, 5s fallback ONLY when SSE disconnected). 9 NEW backend tests + 19 frontend phase-24 tests; vitest 1807 + 1 skipped / ruff clean / ruff format --check clean / ESLint 0 / knip `{"issues":[]}` / vue-tsc only 5 pre-existing PilotPage phase-23 debt errors (baseline, ZERO new). key facts: jsdom vitest EventSource needs addEventListener per named SSE event (onmessage only fires for unnamed frames) + asyncio.Queue per-subscriber cap 100 drop-oldest for slow consumers + in-flight job state re-sync via initial job_state event from disk JSON on client reconnect + 5s fallback polling ONLY when isConnected===false (bridge not source of truth) + EventSource auto-reconnects on transient errors + gate was NON-ZERO at baseline (5 PilotPage vue-tsc errors + pre-existing env pytest fails — verified identical on master). Carryover to Phase 25+: SSE filter/auth/replay + per-chapter preview drawer in PilotLivePanel + batch templates + multi-LLM concurrent batches + usePilotBatch DTO migration to @/api/studio re-export + Phase 114 prod preview regression (accepted). See `docs/superpowers/handoffs/2026-09-02-phase-24-trae-handoff.md`)
  → v23.0 (Phase 23 — Pilot Page feature phase — 19 source commits `c0910284..948173ff` on branch `phase-23-pilot-page`; **first new feature phase since Phase 115 Write Workspace (v15.0)** — closes the longest LingWen dry spell (~10 months between feature phases). Frontend reorganization: extracted embedded Pilot surface from `StudioPage.vue` into dedicated `PilotPage.vue` + 5 components (`PilotStartForm`/`PilotPreflightTable`/`PilotLivePanel`/`PilotCancelDialog`/`PilotHistoryList`) + `usePilotBatch` composable with 3s auto-polling + ETA via `pilot_records.jsonl` join. Backend light touch: `infra/studio_batch_runner.cancel_batch_job()` with SIGTERM + 5s grace + SIGKILL fallback (with `/proc/<pid>/status` zombie-detection via State field check, since `os.kill(pid, 0)` returns success for zombies) + `POST /api/studio/batch/<job_id>/cancel` route + `compute_pilot_eta()` pure helper + `GET /api/studio/batch/history` route. New DTOs: `StudioBatchJobSummary` + `StudioBatchJobListResponse` in `lingwen_shared` (Python + TS codegen). 1 NEW architecture invariant (#48 PilotPage owns batch lifecycle surface; StudioPage no longer hosts Pilot). **vitest 1800 + 1 skipped** (1762 baseline + 38 new) / vue-tsc 0 / ESLint 0 / knip `{"issues":[]}` / ruff clean / ruff format --check 1201 files / lint-imports 3 contracts KEPT (316 files / 1388 deps). **8 lessons**: verify-before-design re-confirmed (N.14 lesson 1) — plan Task 5 used raw `fetch()` + `/api/` prefix (v16.2.1 regression), fixed to `request()` convention + Task 7 referenced non-existent `fetchStudioStartBatchJob` (substituted canonical `studioProductionRun`) + `attachTo: true` is invalid in vue-test-utils v2.4.11 (must be `Element | string`); `<button type="submit">` inside form + `trigger('click')` doesn't simulate form submission in jsdom (use `type="button"` with explicit `@click`); DTO re-export chain requires BOTH `studio.ts` AND `index.ts` (wrapper imports from `@lingwen/dashboard-contracts/shared` → `shared/index.ts`); `DASHBOARD_NAV_ENTRIES` doesn't exist (real nav distributed across `humanFirstNav.js` + `dashboardNavByMode.js` + `navConstants.ts` + `router/index.js`); tsc catches silent test bugs (`StudioBatchJobListResponseDTO` propagation required `index.ts` re-export update); atomic 1-task-per-commit scales to 19 (N.14 lesson 7 re-confirmed). Carryover to Phase 24+: SSE/WebSocket real-time progress (replace 3s polling) + per-chapter preview drawer in PilotLivePanel + batch templates + multi-LLM concurrent batches + Pilot + Insight dashboard integration + batch priority queue + auto-restart on failure + `usePilotBatch` compose DTO migration to `@/api/studio` re-export + Phase 114 prod preview regression (accepted). See `docs/superpowers/handoffs/2026-09-02-phase-23-pilot-page-handoff.md`)
