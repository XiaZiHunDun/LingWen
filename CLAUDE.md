...
> **版本**: v16.5 #N.11.d/e/g (Phase 126 CVG cleanup completion — `RippleListItemResponse.impact_score` exposed (N.11.d) + `cvg_adapter.ripple_storage_to_presentation` propagates + `list_ripples` drops hybrid storage-roundtrip + `cascadeGraphUtils.js`/`useWorkflowSocket.js` JSDoc read presentation-shape fields (`node_id`/`chapter_id`/`source`/`target`/`max_depth`, N.11.e) + `ReferenceGraphResponse.truncated` extended + NEW `cvg_adapter.reference_graph_storage_to_presentation` adapter (computes `by_dimension` from nodes) + `get_reference_graph` endpoint migrated to canonical + storage-shape `ReferenceGraphResponse` class removed from `apps/studio_api/protocols.py` + `_build_reference_graph_response` rewrites directly to canonical + `ImpactGraph.vue` end-to-end migration (N.11.g); 13 commits `c01615ef..c1244097`, vue-tsc 0, ruff 0, 3 NEW architecture invariants (#26-#28), 1733 vitest / 129 shared (+4) / 45 studio_api / vue-tsc 0 / ruff 0 / lint-imports 3 contracts KEPT, #N.13+ 39 `as unknown as` cast cleanup remaining)
