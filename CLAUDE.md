...
> **版本**: v35.0 (Phase 36 P3-ARCHDEBT errors pilot 闭环) · 更新: 2026-09-08
...
- ✅ **v35.0 P3-ARCHDEBT (errors pilot)**（2026-09-08 ff-merge `phase-36-p3-archdebt-errors`）：P3-ARCHDEBT pilot — `infra/errors.py` (1 module, 380 lines, 23 public symbols) → `packages/lingwen-errors/`。14 consumer 迁移 (6 packages: lingwen-core / lingwen-got / lingwen-world-model / lingwen-quality / lingwen-shared / apps.studio_api + 8 intra-infra)；`infra/errors.py` 删除；invariant #51 NEW。5 atomic commits on phase-36-p3-archdebt-errors (C0 spec / C0b plan / C1 scaffold / C2 migrate / C3 delete / C4 invariant+version)。**Validation gates**: lingwen-errors 全部测试 + lingwen-core 68/68 + ruff clean + grep audit 0 行（infra.errors.* 引用清零）。**Carryover closure**: P3-ARCHDEBT 1/5 (errors) → CLOSED; P3-ARCHDEBT remaining 4/5 (paths / project_config / logging_config / studio_registry) → Phase 37+。详见 `docs/superpowers/handoffs/2026-09-08-phase-36-p3-archdebt-errors-handoff.md`。
...
- **架构债（v25.9/v27/v31/v32/v33/v34 推后，Phase 36+ 候选）**：HANDOFF 文档 `latest_decision_queue` 措辞修订; P3-ARCHDEBT: `infra.{paths, project_config, logging_config, errors, studio_registry}` → packages/ (Phase 36+, multi-week, 5 模块)。v32.0 SHIM-CLEANUP 已清 3 个 PHASE-COMPAT shim (infra/subplot + infra/world_model data_structures + master_controller) + 1 fixup commit (5 relative imports); v33.0 LINGWEN-GOT 已迁 `infra.got.*` → `packages/lingwen-got/` + 删除 `infra/got/` 目录 + invariant #49; v34.0 WORLD-MODEL-PACKAGE 已迁 `infra.world_model.*` + `infra/subplot/helpers.py` → `packages/lingwen-world-model/` + 删除 `infra/world_model/` 目录 + invariant #50 + 10 commits。
