...
- Modify: `tests/agent_system/test_master_controller_stub_router_e2e.py` (+4 tests)
...
### Step 1.2: 加 4 个 test_master_controller_stub_router_e2e 测试 (YAML 拓扑 + E2E)

在 `test_master_controller_stub_router_e2e.py` 末尾添加新 class:
...
**文件 4**: `tests/agent_system/test_master_controller_stub_router_e2e.py:231, 298`
```python
# 旧:
assert result["summary"].completed == 5
...
**文件 5**: `tests/agent_system/test_master_controller_stub_router_e2e.py:236, 243-246`
```python
...
pytest tests/agent_system/test_master_controller_stub_router_e2e.py::TestNovelWritingMultiVariantPolish -v
...
+ test_master_controller_stub_router_e2e (4 处: completed 断言 + expected_ids + LLM >= 4→6)
