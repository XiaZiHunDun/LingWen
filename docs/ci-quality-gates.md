...
\* 主门已含 vitest + lint + build；**Playwright live-backend（95 项）** 在 `dashboard-frontend-ci.yml`（`e2e-live` job）blocking；Frontend CI 另含 typecheck + **coverage** + a11y/visual/ui-metrics。
...
**已删除（12.08）**：`dashboard-e2e-live.yml`（与 Frontend CI `e2e-live` 重复）、`.github/workflows/*.yml` 废弃副本。
