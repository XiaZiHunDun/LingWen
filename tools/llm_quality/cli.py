...
def save_report(reports: Dict[int, QualityReport], output_file: Path):
    ...
    for issue in all_issues:
        report_data["summary"]["by_type"][issue.issue_type] = (
            report_data["summary"]["by_type"].get(issue.issue_type, 0) + 1
        )
        report_data["summary"]["by_severity"][issue.severity] = (
            report_data["summary"]["by_severity"].get(issue.severity, 0) + 1
        )
...
def main():
    ...
    parser.add_argument(
        "--check",
        type=str,
        choices=["character", "logic", "foreshadow", "rhythm", "all"],
        default="all",
        help="检测类型",
    )
    ...
    if all_reports:
        output_file = (
            Path(args.output) if args.output else paths.PROJECT_ROOT / "logs" / "llm_quality_report.json"
        )
