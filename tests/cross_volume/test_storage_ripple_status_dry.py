...
def storage_with_applied_ripple(storage):
    ...
    with storage._connect() as conn:
        storage._update_ripple_status_internal(conn, "rip-dry-1", "applied", applied_at)
        conn.commit()
...
class TestUpdateRippleStatusInternal:
    def test_sets_status_and_null_applied_at(self, storage_with_applied_ripple):
        ...
        with storage._connect() as conn:
            storage._update_ripple_status_internal(conn, "rip-dry-1", "pending", None)
            conn.commit()
    ...
    def test_sets_status_and_iso_applied_at(self, storage_with_applied_ripple):
        ...
        with storage._connect() as conn:
            storage._update_ripple_status_internal(conn, "rip-dry-1", "applied", ts)
            conn.commit()
        row = storage.get_ripple_by_id("rip-dry-1")
    ...
    def test_accepts_all_five_status_values(self, storage):
        for idx, status in enumerate(("pending", "applied", "rejected", "failed", "created")):
            ...
            with storage._connect() as conn:
                storage._update_ripple_status_internal(conn, ripple_id, status, None)
                conn.commit()
...
class TestRollbackAndResetUseInternalHelper:
    ...
    def spy_helper(self, monkeypatch):
        ...
        monkeypatch.setattr(RippleStorage, "_update_ripple_status_internal", _spy)
        return calls

    def test_rollback_delegates_to_internal_helper(self, storage_with_applied_ripple, spy_helper):
        storage, _ = storage_with_applied_ripple
        storage.rollback_ripple("rip-dry-1", actor="user", origin="ui", reason="undo")
        assert spy_helper == [("rip-dry-1", "pending", None)]

    def test_reset_for_test_delegates_to_internal_helper(self, storage_with_applied_ripple, spy_helper):
        storage, _ = storage_with_applied_ripple
    ...
    def test_rollback_and_reset_share_same_db_shape_for_pending(self, tmp_path):
