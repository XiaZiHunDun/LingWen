...
by ``lingwen_llm_service`` at module load time, so importing it here ensures
test files that construct these tools get a working adapter.

In production, the application startup imports ``lingwen_llm_service`` (or any
...
# Trigger factory registration: lingwen_llm_service registers itself as the
