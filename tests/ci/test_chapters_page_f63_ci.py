...
class TestChaptersPageF63Contract:
    def test_chapters_page_vue_exists(self):
        vue = NOVEL_FACTORY / "apps" / "dashboard" / "src" / "pages" / "ChaptersPage.vue"
        assert vue.is_file()
    ...
    def test_app_wires_chapters_page(self):
        page = NOVEL_FACTORY / "apps" / "dashboard" / "src" / "pages" / "ProducePage.vue"
        text = page.read_text(encoding="utf-8")
        assert "ChaptersPage" in text
        assert "章节管理 - 开发中" not in text

    def test_chapters_page_unit_test_exists(self):
        spec = NOVEL_FACTORY / "apps" / "dashboard" / "tests" / "unit" / "chapters-page.spec.ts"
        assert spec.is_file()
