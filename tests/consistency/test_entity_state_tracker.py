...
def test_location_state_creation():
    loc = LocationState(location="厨房", inDoor=True, previous_location="客厅", transition_type="walked")
...
def test_entity_state_apply_action():
    entity = EntityState(entity_id="林夜", entity_type="character", alive=True)
...
def test_knowledge_state_share_secret():
    knowledge = KnowledgeState(known_secrets={"父亲的秘密"}, shared_with={"林夜": {"父亲的秘密"}})
...
def test_relationship_state():
    rs = RelationshipState(
        trust_level=0.8, emotional_bond=0.5, status="friends", last_status_change_chapter=10
    )
...
def test_capability_state():
    cap = CapabilityState(abilities={"剑气", "御剑术"}, mastery_level={"剑气": 0.9, "御剑术": 0.5})
