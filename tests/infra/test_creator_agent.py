...
class TestCreatorAgent:
    @pytest.mark.asyncio
    async def test_chapter_scope_returns_candidates(self, project_root: Path) -> None:
    ...
    @pytest.mark.asyncio
    async def test_advice_only_when_style_strength_zero(self, project_root: Path) -> None:
        result = await run_creator_agent_plan(
            project_root,
    ...
    @pytest.mark.asyncio
    async def test_selection_requires_text(self, project_root: Path) -> None:
        with pytest.raises(ValueError, match="selection_text"):
            await run_creator_agent_plan(
                project_root,
                action="prompt",
                action_label="test",
    ...
    @pytest.mark.asyncio
    async def test_goal_tag_adjusts_advice(self, project_root: Path) -> None:
        result = await run_creator_agent_plan(
            project_root,
            action="path:faster",
            action_label="加快节奏",
    ...
    @pytest.mark.asyncio
    async def test_editor_lens_returns_annotations(self, project_root: Path) -> None:
        result = await run_creator_agent_plan(
            project_root,
            action="path:faster",
            action_label="加快节奏",
    ...
    @pytest.mark.asyncio
    async def test_reviewer_lens_advice_focus(self, project_root: Path) -> None:
        result = await run_creator_agent_plan(
            project_root,
    ...
    @pytest.mark.asyncio
    async def test_stream_yields_chunks_and_done(self, project_root: Path) -> None:
        events: list[dict] = []
