...
def anye_project(tmp_path: Path, monkeypatch):
    monkeypatch.setattr("lingwen_studio_batch_runner._jobs_dir", lambda: tmp_path / "jobs")
...
def test_submit_idle_starts_immediately(anye_project, monkeypatch):
    with (
        patch(
            "lingwen_studio_batch_runner.subprocess.Popen",
...
def test_advance_promotes_head_and_keeps_job_id(anye_project, monkeypatch):
    from lingwen_studio_batch_runner import _save_job
...
def test_poll_auto_advances_after_terminal(anye_project, monkeypatch):
    ...
    with (
        ...
        patch("lingwen_studio_batch_runner._process_running", return_value=False),
        patch("lingwen_studio_batch_runner._read_exit_code", return_value=0),
    ):
