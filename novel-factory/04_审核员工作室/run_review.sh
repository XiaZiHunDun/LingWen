...
选项:
  --mode <full|lean|solo>   设置审核强度模式（默认: full）
                             示例: REVIEW_MODE=lean ./run_review.sh batch ch001-ch010
...
审核模式说明:
  full  - 全部10+1维度，10个审核员，最高标准（默认）
  lean  - 6个核心维度，5个审核员，快速周转
  solo  - 2个基础维度，自审，最快速度
EOF
}
...
if [[ ! "$REVIEW_MODE" =~ ^(full|lean|solo)$ ]]; then
    echo -e "${RED}[ERROR]${NC} Invalid review mode: $REVIEW_MODE"
    echo "Valid modes: full, lean, solo"
    exit 1
fi

log_info "Review mode: $REVIEW_MODE"
