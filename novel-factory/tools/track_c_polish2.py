...
LOG_FILE = PROJECT_ROOT / "logs" / "minimax_review" / "track_c_polish2.log"

def log(msg):
    timestamp = datetime.now().strftime('%H:%M:%S')
    line = f"[TrackC-P2] [{timestamp}] {msg}"
    print(line, flush=True)
    with open(LOG_FILE, 'a') as f:
        f.write(line + "\n")

def main():
    chapters = list(range(241, 361))

    log("=" * 60)
    log("AI痕迹消除第二轮 - Track C (ch241-ch360)")
    log("=" * 60)
    ...
    log("=" * 60)
    log(f"Track C 第二轮完成: {modified_count}/{len(chapters)} 章有修改")
    log("=" * 60)
