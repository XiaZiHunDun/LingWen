...
LOG_FILE = PROJECT_ROOT / "logs" / "minimax_review" / "track_c.log"
LOG_FILE.parent.mkdir(parents=True, exist_ok=True)

def log(msg):
    timestamp = datetime.now().strftime('%H:%M:%S')
    line = f"[TrackC] [{timestamp}] {msg}"
...
def main():
    ...
    chapters = list(range(241, 361))  # ch241-ch360

    log("=" * 60)
    log("Track C 修复流水线启动 (ch241-ch360)")
    log("=" * 60)
    ...
    log("\n" + "=" * 60)
    log(f"Track C 完成 - 耗时 {elapsed/60:.1f} 分钟")
    log("=" * 60)

    result = {
        "track": "C",
        "chapters": "ch241-ch360",
        "elapsed_minutes": elapsed / 60,
        "timestamp": datetime.now().isoformat()
    }
    result_file = PROJECT_ROOT / "logs" / "minimax_review" / "track_c_result.json"
