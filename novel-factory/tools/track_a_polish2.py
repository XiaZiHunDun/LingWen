...
LOG_FILE = PROJECT_ROOT / "logs" / "minimax_review" / "track_a_polish2.log"

def log(msg):
    timestamp = datetime.now().strftime('%H:%M:%S')
    line = f"[TrackA-P2] [{timestamp}] {msg}"
...
def main():
    ...
    log("=" * 60)
    log("AI痕迹消除第二轮 - Track A (ch001-ch120)")
    log("=" * 60)
    ...
    log("=" * 60)
    log(f"Track A 第二轮完成: {modified_count}/{len(chapters)} 章有修改")
    log("=" * 60)
