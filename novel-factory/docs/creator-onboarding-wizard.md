...
- UI：`?nav=write` 创作页顶部 `data-testid="onboarding-wizard-panel"`（`nav=creator` 仍兼容）
- **Deep link**：`?nav=write&wizard=1` 自动展开向导并滚动到面板
- **单步 deep-link**：`?nav=write&wizard=1&step=volume` 高亮并滚动到对应步骤
- **分享链接**：`?nav=write&wizard=1&done=pillars,volume` 打开时合并已完成步骤
...
- **向导 deep-link**：`?nav=write&wizard=1`
...
- **向导分享链接**：`?nav=write&wizard=1&done=pillars,volume` · `POST /api/creator/onboarding/progress/apply-share`
...
- **diff 变更计数**：`volume_plan_diff_change_count` · 卷纲 diff 标题旁显示变更条数
- **batch 预算提示**：`batch_history_budget_hint` · 历史任务回填范围时同步提示预算
- **Studio 徽章色标**：`studio_creation_mode_badge_tint` · 工作室模式徽章琥珀色强调

## v5.9 关联能力

- **diff 类型筛选**：`volume_plan_diff_type_filter` · 按 changed / added / removed 过滤变更列表
- **batch 耗时**：`batch_history_duration` · 历史任务展示 started→finished 耗时
- **徽章图例**：`creation_mode_badge_legend` · 页头说明陪伴/推进/工作室徽章色标

## v6.0 关联能力

- **三模式预览**：`creation_mode_switch_preview` · 页头并排展示陪伴/推进/工作室摘要并高亮当前模式
- **diff 导出**：`volume_plan_diff_export` · 导出当前卷纲 diff 为 JSON
- **batch 成功率**：`batch_history_success_rate` · batch 历史面板展示 completed 占比

## v6.1 关联能力

- **diff 卷筛选**：`volume_plan_diff_volume_filter` · 按卷 label 过滤 diff 变更
- **batch 平均耗时**：`batch_history_avg_duration` · 历史任务平均运行分钟数
- **YAML 片段复制**：`creation_mode_yaml_snippet` · 三模式预览旁一键复制 `creation_mode:` 配置

## v6.2 关联能力

- **diff 导出含大纲**：`volume_plan_diff_export_outline` · 导出 JSON 附带 global_outline_excerpt / lines
- **batch 失败趋势**：`batch_history_failure_trend` · 失败率与近期相对前期升降趋势
- **文档一键打开**：`creation_mode_switch_doc_open` · 模式切换文档链接新窗口打开

## v6.3 关联能力

- **diff 变更高亮导出**：`volume_plan_diff_export_highlight` · 导出 JSON 附带 highlighted_changes / highlighted_outline_lines
- **batch 按周汇总**：`batch_history_weekly_summary` · ISO 周维度统计成功/失败次数
- **能力对照表**：`creation_mode_capability_matrix` · 陪伴/推进/工作室核心能力矩阵

## v6.4 关联能力

- **diff Markdown 导出**：`volume_plan_diff_export_markdown` · 导出可读 Markdown 变更报告
- **batch 按月汇总**：`batch_history_monthly_summary` · YYYY-MM 维度统计成功/失败次数
- **切换引导动画**：`creation_mode_switch_guide_animation` · 三模式预览依次高亮引导

## v6.5 关联能力

- **diff 邮件分享**：`volume_plan_diff_export_email_share` · mailto 打开邮件客户端附带 diff 正文
- **batch 成功率折线图**：`batch_history_success_rate_chart` · 累计成功率 SVG 趋势
- **onboarding 步骤联动**：`creation_mode_onboarding_step_link` · 三模式预览与向导步骤互链

## v6.6 关联能力

- **diff PDF 导出**：`volume_plan_diff_export_pdf` · 下载文本型 PDF 变更报告
- **batch 失败原因**：`batch_history_failure_reason_label` · 失败任务展示 failure_reason 标签
- **切换确认框**：`creation_mode_switch_confirm_dialog` · 非当前模式 YAML/向导操作前确认

## v6.7 关联能力

- **diff 打印预览**：`volume_plan_diff_export_print_preview` · 内嵌预览 diff 并一键打印
- **batch 状态堆叠图**：`batch_history_status_stack_chart` · completed/failed/running 堆叠条
- **切换历史记录**：`creation_mode_switch_history` · localStorage 记录最近 5 次模式切换操作

## v6.8 关联能力

- **diff ZIP 打包**：`volume_plan_diff_export_zip` · 一键导出 JSON + Markdown + PDF 压缩包
- **batch 耗时分布**：`batch_history_duration_distribution` · 按耗时区间柱状分布图
- **切换撤销提示**：`creation_mode_switch_undo_hint` · 模式切换后展示恢复 YAML 提示

## v6.9 关联能力

- **diff 分享链接**：`volume_plan_diff_export_share_link` · 复制带 `#creator-diff=` 哈希的分享 URL
- **batch 并发运行图**：`batch_history_concurrency_chart` · 时间轴并发峰值与分布柱状图
- **切换快捷键**：`creation_mode_switch_hotkey` · Alt+Shift+1/2/3 复制三模式 YAML

## v6.10 关联能力

- **分享链接解析预览**：`volume_plan_diff_share_link_preview` · 打开 `#creator-diff=` 链接自动解析变更列表
- **batch 队列深度图**：`batch_history_queue_depth_chart` · 排队等待阶段重叠深度与峰值
- **切换语音朗读**：`creation_mode_switch_speech` · 复制 YAML 时朗读目标模式名称

## v6.11 关联能力

- **分享一键应用**：`volume_plan_diff_share_link_apply` · v2 分享 token 含 draft 卷纲，解析后可写入编辑区
- **batch 吞吐率图**：`batch_history_throughput_chart` · 按任务展示章/分钟吞吐柱状图
- **切换触觉反馈**：`creation_mode_switch_haptic` · 复制 YAML 时短振动提示

## v6.12 关联能力

- **分享应用确认**：`volume_plan_diff_share_link_apply_confirm` · 应用分享卷纲前二次确认
- **batch 成本效率**：`batch_history_cost_efficiency_chart` · 按任务展示 $/章柱状图
- **减动画偏好**：`creation_mode_switch_reduced_motion` · 尊重 `prefers-reduced-motion` 关闭引导动画

## v6.13 关联能力

- **分享 token 校验**：`volume_plan_diff_share_token_validation` · 损坏/不支持版本友好提示
- **batch 重试堆叠**：`batch_history_retry_rate_stack` · 首次成功/重试成功/失败堆叠条
- **切换 ARIA 公告**：`creation_mode_switch_aria_live` · 模式切换 `aria-live` 播报

## v6.14 关联能力

- **分享冲突合并**：`volume_plan_diff_share_link_merge` · 本地与分享卷纲冲突时合并向导
- **章节失败热力**：`batch_history_chapter_failure_heatmap` · 按章格展示失败热力
- **模式固定侧栏**：`creation_mode_preview_pinned_sidebar` · 三模式预览 sticky 侧栏

## v7.0 里程碑能力

- **分享闭环 E2E**：`volume_plan_diff_share_link_e2e` · 解析→应用→保存步骤条 + Playwright live 用例
- **batch 运维摘要**：`batch_history_ops_summary` · 折叠区收束全部可视化图表
- **无障碍验收清单**：`creation_mode_accessibility_checklist` · 快捷键/语音/触觉/ARIA/减动画/侧栏一览

## v7.0+ 产品级能力

- **分享协作 v2**：`volume_plan_diff_share_collab_v2` · 分享 token v3 含卷级批注 `n` · `GET/PUT /api/creator/diff-collab-notes`
- **beta 文档包**：`docs/creator-beta-pack/` · `bash scripts/verify-creator-beta-pack.sh`
- **changelog 解耦**：`docs/creator-changelog.md`（创作者）· `docs/studio-changelog.md`（Studio）
