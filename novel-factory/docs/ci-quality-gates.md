...
\* 主门已含 vitest + lint + build；**Playwright live-backend（66 项）** 在 `dashboard-frontend-ci.yml`（`e2e-live` job）blocking；Frontend CI 另含 typecheck + **coverage** + a11y/visual/ui-metrics。
...
| Workflow | 触发 | 说明 |
|----------|------|------|
| **Dashboard Frontend CI** | `dashboard/frontend/**` 变更 | lint · typecheck · **coverage** · build · **e2e-live（66）** · a11y · visual · ui-metrics |
