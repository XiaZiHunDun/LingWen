...
> **状态**: v8 进行中; **推荐 F79** Manual batch 3 章 pilot
...
**v7 清空后缺口:** 批量 3–10 章 **尚未真实 LLM 执行**; Settings **预算只读** (API PUT 已有); Analytics 未聚合 batch 成本; 360+ 章规模化运维未规划。
...
### F79. Phase 9.87 — Manual batch pilot（3 章）

- **目标**: `chapter_production_batch --start-chapter 361 --max-chapters 3` 真实 LLM
- **manual gate**: 需 `LINGWEN_REAL_LLM=1` + provider API key
- **交付**: batch summary JSON + runbook §15 操作记录模板
- **estimated**: 2–4h（含人工审）
- **Out of scope**: >10 章单次 batch · 改 workflow YAML

### F80. Phase 9.88 — Batch 生产 dry-run / preflight 增强

- **目标**: batch `--dry-run` 或增强 preflight 输出（章节范围、预估成本上限）
- **依赖**: F79 有真实记录后校准
- **estimated**: 2–3h
...
1. **F79** Manual batch 3 章 pilot（需 API key）
2. F81 Analytics rollup · F80 dry-run · F82 e2e 确认
