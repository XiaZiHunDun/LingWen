...
### F75. Phase 9.83 — 决策中心 ↔ 章节 deep link ✅ **DONE**

- **交付**: `useDashboardNav` URL 桥接 · `chapterDecisionLink.js` · ChaptersPage「决策」按钮 · DecisionsPage focus banner
- **后端**: harvest 决策 `context.chapter_num` 来自 workflow initial_inputs
...
### F76. Phase 9.84 — Remote e2e-live CI 首绿验证 ✅ **DONE**

- **交付**: runbook §11.2 · `e2e-live-remote-checklist.sh` · workflow job summary step
- **manual gate**: GitHub Actions → Run workflow（本环境无 `gh` CLI）
...
v7 清单已清空。可选：GitHub Actions 手动 Run workflow 确认 · 批量生产新 roadmap。
...
- [x] F72 manual pilot ch360 (MiniMax M2.7)
- [x] F76 remote checklist + workflow summary
