...
> **状态**: v7 开启; **推荐 F72** Manual 1 章 pilot 执行
...
### F72. Phase 9.80 — Manual 1 章 pilot 执行与记录 🟡 **脚手架 DONE**

- **目标**: 在有 API key 的环境跑通 **1 章** `novel_writing` + 填 `chapter-pilot-record` 模板
- **交付**:
  - `chapter_production_pilot.py --save-record PATH` 写 JSON 记录 ✅
  - runbook §13「人工审章 checklist」 ✅
  - `docs/templates/chapter-pilot-record.stub.example.json` ✅
  - pytest: record schema 校验 + `--save-record` dry-run ✅
- **manual gate**: 主公配置 API key 后执行（本环境 preflight: api_key ❌）
- **manual gate**: 主公配置 `ANTHROPIC_API_KEY` 或 `MINIMAX_API_KEY` 后执行
...
### F73. Phase 9.81 — 批量章节生产 runner（3–10 章）

- **目标**: 顺序跑 N 章, stop-on-fail, per-chapter cost 汇总
- **交付**:
  - `infra/agent_system/chapter_production_batch.py` + CLI
  - `--max-chapters` · `--start-chapter` · `--budget-usd` hard stop
  - batch summary JSON（每章 PilotResult 数组）
  - runbook §14 + pytest（stub/mock MC, 0 真 LLM in CI）
- **estimated**: 4–6h
- **依赖**: F72 manual gate 通过（或 stub 等价验收）
- **Out of scope**: 并行多章 · 359 章全量 overnight
...
### F74. Phase 9.82 — ChaptersPage 生产历史面板

- **数据源**: `GET /api/production-records`（读 `infra/.state/pilot_records/` 或等价）
- **UI**: 每章 badge + 最近 run 摘要（cost / backfill / memory source）
- **只读**; 写入仍走 CLI pilot/batch
...
1. **F72** Manual 1 章 pilot 执行（需 API key; 脚手架 `--preflight-only` 已可用）
2. **F73** 批量 runner（F72 通过后）
3. **F74** ChaptersPage 生产历史（有 record 文件后）
