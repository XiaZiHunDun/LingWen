...
> **版本**: v9.12 (文档与代码同步版)
> **更新 (2026-06-03)**：v9.11 LLM 增强检测器并入主分支,代码与文档同步刷新,清理 P5-4 死代码与 R1-006/011 魔法数。
...
novel-factory/
├── 01_灵感库/          # 灵感收集、概念发散
├── 02_作家工作室/      # 作家Agent工作区（作家A-J角色池）
├── 03_内容仓库/        # 内容存储（大纲+正文），仅存储+索引更新
│   ├── 01_全文总体大纲/ # 全局大纲
│   ├── 02_卷大纲/      # 3卷大纲
│   ├── 03_阶段大纲/    # 15+阶段大纲
│   └── 04_正文/
...
│       ├── ch001_大纲.md
│       └── ...
├── 04_审核员工作室/    # 审核Agent工作区（审核员A-J角色池）
├── 05_模拟读者池/      # 读者反馈Agent（读者A-T角色池）
├── 06_意见仓库/        # 审核意见、修改记录存档
├── 07_汇总仓库/        # ⚠️ 正文汇总（合并章节→全文），不是索引
│   ├── 汇总主笔/       # 汇总文本生成
│   ├── 汇总编辑/       # 汇总审核润色
│   ├── 汇总校验/       # 汇总一致性验证
│   ├── 全文正文/       # 最终全文（星陨纪元_全文正文_v9.10.md）
│   ├── 卷正文/         # 各卷正文
│   └── 阶段正文/       # 各阶段正文
├── 08_已发布/          # 发布定稿存档
...
**"汇总"在两个仓库含义不同**：
- `03_内容仓库/update_index.py` → 更新索引（文件的元数据目录）
- `07_汇总仓库/run_summary.sh merge` → 合并正文（把ch001-ch360合并成完整小说）

## 核心文件

- `.state/workflow.db` - **SQLite状态数据库**（v9.1+），所有进度由此驱动
- `.state/workflow.db.journal` - SQLite日志文件
- `hooks.yaml` - 事件触发配置
- `infra/state/workflow_validator.py` - 状态转换校验（三条铁律之一）
- `.skills/` - Agent角色池配置
- ~~`workflow_state.json`~~ - **已废弃**，请勿手动修改（已被`.state/workflow.db`替代）

> **注意**：`workflow_state.json`标注为DEPRECATED，所有状态操作应通过SQLite数据库进行。
...
**总章节数**：359章（ch045已删除，实际文件359个正文+359个大纲）
**最新版本**：v9.12
**发布状态**：已发布（2026-06-03），v9.11 LLM 增强检测器已合并(v9.10 之后)
