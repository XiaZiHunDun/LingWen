...
> **版本**: v11.29 (创作者 v6.11, 2026-06-24)  
...
|----|-------------------|-----------------|----------------|
| **A · 卷纲 diff / 分享** | 导出→分享→解析→**应用**→保存闭环 | JSON/MD/PDF/ZIP/邮件/链接/解析预览 | v6.11 应用 ✅ · v6.12 应用前确认 · v6.13 token 校验 · v6.14 冲突合并 · **v7.0 E2E** |
| **B · batch 历史可视化** | 运维一眼看懂（耗时/并发/队列/吞吐/成本） | 成功率/周月汇总/折线/堆叠/分布/并发/队列 | v6.11 吞吐 ✅ · v6.12 成本效率 · v6.13 重试率 · v6.14 失败热力 · **v7.0 仪表盘收束** |
| **C · 三模式切换体验** | 切换无障碍包（预览/快捷键/语音/触觉/ARIA） | 预览/YAML/确认/历史/撤销/快捷键/语音 | v6.11 触觉 ✅ · v6.12 减动画偏好 · v6.13 ARIA · v6.14 固定侧栏 · **v7.0 无障碍收束** |

| 版本 | A 柱 | B 柱 | C 柱 | 状态 |
|------|------|------|------|------|
| **v6.11** | 分享 v2 token + 一键应用卷纲 | 吞吐率图（章/分） | `navigator.vibrate` 触觉 | ✅ |
| **v6.12** | 应用前 diff 二次确认对话框 | 成本效率图（$/章） | `prefers-reduced-motion` 关闭引导动画 | 待做 |
| **v6.13** | 分享 token 版本/损坏提示 | 重试成功率堆叠条 | `aria-live` 模式切换公告 | 待做 |
| **v6.14** | 分享与本地卷纲冲突合并向导 | 章节失败率热力格 | 三模式预览固定侧栏 | 待做 |
| **v7.0** | Playwright：分享链接→应用→保存 | batch 历史面板「运维摘要」折叠区 | 无障碍验收清单 + verify 脚本扩展 | **里程碑** |
...
**v6.12 候选（下一版三柱）**：卷纲 diff 分享应用确认框 · batch 历史成本效率图 · 三模式减动画偏好联动
...
1. **人工抽检覆写** — [`prose-calibration-log.md`](novel-factory/docs/prose-calibration-log.md)「留/删/疑」
