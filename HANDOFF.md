...
> **版本**: v9.61 (Phase 9.58-9.61 F49-F52, 2026-06-11)
> **更新 (2026-06-11)**: F49 pre-commit pytest smoke opt-in ✅ · F52 audit retention purge ✅; baseline **2648 pytest + 355 vitest**; 推荐 **F54** 或 **F53**.
...
| **后端** | Python 3.13 · FastAPI · SQLite (`.state/*.db`) · Pydantic v2 · pytest 2648 passed |
...
| **总测试** | **3003+** (2648 pytest + 355 vitest + 27 pytest skip) |
...
| **下一期推荐** | **F54** (backfill 增量) 或 **F53** (audit WS push) — 见 v4 roadmap |
...
**推荐下一项**: **F54** (backfill 增量 workflow hook) 或 **F53** (audit WS push) — Track B CVG 见 v4 doc.
...
- [ ] 跑 `pytest -q` 验证 baseline 2648 passed (~2min)
...
- [ ] 选 1 个 v4 item (推荐 **F54 backfill 增量** 或 **F53 audit WS push**) → brainstorming → writing-plans
...
```bash
# === Tests ===
cd novel-factory && pytest -q                                    # 2648 tests, ~2min
