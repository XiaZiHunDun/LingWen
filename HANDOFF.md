...
| **Phase 15.0 P3 一致性 · T2** | ⏳ | `3fd59a87` spec/plan/tasks 三文档 (443 行) — SQLite 8 类 (2986 行 / 179 db_path) → `infra/persistence/{registry,connection,schemas,paths,bootstrap}.py` · dashboard singleton 走 `get("ripple")` · 删 `dashboard/helpers/reading_power_db.py` 截断副本 · 旧 backend DeprecationWarning · ≥ 26 新测试 in `tests/persistence/` · 9 实施 commits 待主公 approve |
...
**v10 roadmap**: `docs/superpowers/plans/2026-06-16-roadmap-v10-studio.md`
...
| 10.26b | [`docs/eight-books-reading-guide.md`](docs/eight-books-reading-guide.md) 通读索引 | ✅ |
...
| 10.31b | [`docs/primary-revision-book.md`](docs/primary-revision-book.md) 通读 + 改稿流程 | ✅ |
...
| 10.36b | [`jinghai-external-release.md`](docs/jinghai-external-release.md) 对外一包 | ✅ |
| 10.36c | [`studio-demo-record-ready.md`](docs/studio-demo-record-ready.md) 录屏清单 | ✅ |
...
| 10.38c | 可选 LLM CI job + [`ci-quality-gates.md`](docs/ci-quality-gates.md) | ✅ |
| 10.38d | `run-primary-revision-verify.sh` + agency 规则扩充 | ✅ |
| 10.38e | `sync-handoff-baseline.sh` · E2E 超时 30s | ✅ |
| 10.38f | [`roadmap-v11-engineering.md`](docs/superpowers/plans/2026-06-19-roadmap-v11-engineering.md) | ✅ |
...
| # | 任务 | 状态 |
|---|------|------|
| 11.22a | [`prose-rubric-v1.md`](docs/prose-rubric-v1.md) | ✅ |
| 11.22b | [`top-tier-studio-gap-v1.md`](docs/top-tier-studio-gap-v1.md) KPI | ✅ |
...
| 10.39d | [`second-primary-revision-huiyu.md`](docs/second-primary-revision-huiyu.md) | ✅ |
...
**对外分发入口**：`docs/trial-read-index.md`
...
- [ ] 读本 HANDOFF.md (3 分钟)
- [ ] 读 `CLAUDE.md` (主控 agent prompt 模板, 5 分钟)
- [ ] 读 `docs/superpowers/plans/2026-06-11-followup-roadmap-v9-post-9.90.md` (v9 已完成, 5 分钟)
- [ ] 读 auto-memory `phases-8-dashboard-c.md` (最近 phase 详细, 10 分钟)
- [ ] 读 [`studio-production-dod.md`](docs/studio-production-dod.md)（真实 1 章 pilot 标准）
- [ ] 读 [`ci-quality-gates.md`](docs/ci-quality-gates.md) §本地最小验证 + §MiniMax API 成本
...
# === CLI ===
python lingwen.py --help                        # 列出所有 subcommand
python lingwen.py status                        # workflow status
python lingwen.py cascade <ripple_id>          # cascade preview
python -m infra.agent_system.chapter_golden_path  # stub golden path (F59)
```
...
- **CLAUDE.md 主控 prompt**: `CLAUDE.md` — 必读, 5 核心 Agent + 22 步 workflow + 强制反馈循环
...
切换工具如果新工具能读 `~/.claude/projects/.../memory/`, 直接 read MEMORY.md 起步; 不能读就复制本 HANDOFF.md + 关键 phase docs 到 `docs/HANDOFF-HISTORY/`。
