        ...
        if word in text:
            found.append(
                (
                    ...
                    else ("negative" if word in NEGATIVE_EMOTIONS else "neutral"),
                )
            )
    return found
    ...
    for i, line in enumerate(lines):
        ...
        if len(emotions_line) >= 2:
            ...
            if "positive" in emotion_types and "negative" in emotion_types:
                shifts.append(
                    {
                        "before": [e[0] for e in emotions_line if e[1] == "negative"][0]
                        if any(e[1] == "negative" for e in emotions_line)
                        else "",
                        "after": [e[0] for e in emotions_line if e[1] == "positive"][0]
                        if any(e[1] == "positive" for e in emotions_line)
                        else "",
...
def check_emotion_standards(chapter_path: str) -> Tuple[bool, List[str]]:
    ...
    if total_scenes > 0:
        ...
        violations.append("[S4.2] 未检测到足够的情感层次描写")
    ...
    if positive_count == 0 and negative_count == 0:
        violations.append("[S4.4] 章节缺少明显情绪描写，无法判断节奏健康度")
    ...
    if len(emotion_shifts) >= 2:
        violations.append("[S4.3] 人工审核: 请检查章节是否包含≥2个有效情感共鸣触发点")
    ...
    if has_foreshadowing:
        violations.append("[S4.5] 人工审核: 检测到情感伏笔线索，请确认是否在±15章内回收")
