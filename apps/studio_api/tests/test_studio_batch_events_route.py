...
def test_events_route_returns_404_for_missing_job(client):
    with patch("lingwen_studio_batch_runner._load_job", return_value=None):
...
def test_events_route_returns_200_with_sse_headers(client):
    with patch("lingwen_studio_batch_runner._load_job", return_value=_job(job_id="j1")):
...
def test_events_route_sends_initial_job_state_event(client):
    with patch("lingwen_studio_batch_runner._load_job", return_value=_job(job_id="j3")):
...
def test_events_route_terminal_job_closes_stream_with_terminal_event(client):
    ...
    with patch("lingwen_studio_batch_runner._load_job", return_value=job):
...
def test_events_route_403_when_batch_disabled(client, monkeypatch):
    ...
    with patch("lingwen_studio_batch_runner._load_job", return_value=_job(job_id="j5")):
        resp = client.get("/api/studio/batch/j5/events")
    assert resp.status_code == 403


def test_events_route_400_for_unknown_event_type(client):
    with patch("lingwen_studio_batch_runner._load_job", return_value=_job(job_id="j6")):
...
def test_events_route_403_for_slug_mismatch(client):
    with patch("lingwen_studio_batch_runner._load_job", return_value=_job(job_id="j7", slug="my-project")):
        resp = client.get("/api/studio/batch/j7/events", params={"slug": "other-project"})
    assert resp.status_code == 403


def test_events_route_403_for_mode_mismatch(client):
    with patch("lingwen_studio_batch_runner._load_job", return_value=_job(job_id="j8", mode="pilot")):
...
def test_events_route_event_types_filter_applied(client):
    ...
    with patch("lingwen_studio_batch_runner._load_job", return_value=_job(job_id="j10", status="completed")):
