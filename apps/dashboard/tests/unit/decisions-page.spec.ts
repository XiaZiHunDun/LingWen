...
describe('DecisionsPage (page-level) — Phase 8.39', () => {
  ...
  test('resolved tab shows empty state then expand', async () => {
    const wrapper = mount(DecisionsPage)
    await flushPromises()
    const tabs = wrapper.findAll(byTestid('tab-btn'))
    await tabs[1].trigger('click')
    await flushPromises()
    expect(wrapper.text()).toContain('暂无已解决决策')
    const expandBtn = wrapper.find(byTestid('expand-toggle'))
  ...
  test('decision cards render from store data', async () => {
    ...
    const wrapper = mount(FreshDecisionsPage)
    await flushPromises()
    await wrapper.findAll(byTestid('tab-btn'))[1].trigger('click')
    await flushPromises()
    await wrapper.find(byTestid('expand-toggle')).trigger('click')
    await flushPromises()
  ...
  test('closed tab expand shows cancelled decisions', async () => {
    ...
    const wrapper = mount(FreshDecisionsPage)
    await flushPromises()
    await wrapper.findAll(byTestid('tab-btn'))[2].trigger('click')
    await flushPromises()
    await wrapper.find(byTestid('expand-toggle')).trigger('click')
    await flushPromises()
    expect(wrapper.find('[data-testid="decision-card"]').exists()).toBe(true)
