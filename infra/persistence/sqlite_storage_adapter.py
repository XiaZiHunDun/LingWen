...
v16.5 #3 (DP-03 full fix): this is the ONLY file in ``infra/`` that
imports ``sqlite3``. All other infra modules obtain their connections
through the ``StoragePort`` Protocol from ``lingwen_shared.ports.storage``
via ``with_connection`` / ``with_transaction`` callbacks, so that the
import-linter ``no_concrete_sqlite3_in_business_code`` contract can
eventually be expanded to cover ``apps/`` as well as ``lingwen_creator``.

Architecture invariant: this module is the ONLY allowed
``import sqlite3`` site in ``infra/``. Regression-tested by
``tests/persistence/test_sqlite_storage_adapter.py``.
"""
