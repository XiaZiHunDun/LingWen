...
class CrossVolumeReferenceGraph:
    ...
    def detect_cycles(self) -> list[list[str]]:
        """Phase 14.0 T4: 检测图中的所有循环路径（DFS 着色法）。

        Returns:
            循环路径列表，每条路径是节点ID序列（起点=终点）。
        """
        ...
        def dfs(node_id: str) -> None:
            color[node_id] = GRAY
            path.append(node_id)
            for neighbor_id in self._adjacency_by_node.get(node_id, {}):
                if color.get(neighbor_id) == WHITE:
                    dfs(neighbor_id)
                elif color.get(neighbor_id) == GRAY:
    ...
    def get_cycle_statistics(self) -> dict:
        """Phase 14.0 T4: 获取图的循环统计信息。

        Returns:
            包含循环数量、平均长度、最长/最短循环等指标的字典。
        """
