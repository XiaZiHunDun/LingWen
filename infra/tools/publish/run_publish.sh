...
#-------------------------------------------------------------------------------
show_help() {
    echo -e "${BLUE}=== 灵文 · 工业化小说生产系统 - 发布管理 ===${NC}"
    echo ""
    echo "用法: $0 <命令> [参数]"
    echo ""
    echo "命令:"
    echo "  check <scope> [range]  检查待发布内容"
    echo "                         scope: chapter|volume|all"
    echo "                         例: check chapter ch001-ch010"
    echo "  release <scope> <id> [version]  执行发布"
    echo "                         scope: chapter|volume|all"
    echo "                         例: release volume 1 v5.1"
    echo "  list                   列出所有版本"
    echo "  diff <v1> <v2>         对比两个版本"
    echo "  diff-versions <v1> <v2>  对比releases目录版本"
    echo "  archive <版本>         归档指定版本"
    echo "  merge <版本> [--range <start-end>]  合并正文"
    echo ""
    echo "示例:"
    echo "  $0 check chapter ch001-ch010"
    echo "  $0 release volume 1 v5.1"
    echo "  $0 list"
    echo "  $0 diff v4.0 v5.0"
}
...
do_release() {
    local scope="$1"
    local identifier="$2"
    local version="${3:-v5.1}"

    if [[ -z "$scope" || -z "$identifier" ]]; then
        echo -e "${RED}错误: 请指定范围和标识符，例如: release volume 1${NC}"
        exit 1
    fi

    echo -e "${BLUE}=== 执行发布: $version ===${NC}"
    echo ""

    # 设置发布目录
    local RELEASE_DIR="$SCRIPT_DIR/../releases/${version}-$(date '+%Y-%m-%d')"
    local novel_name="星陨纪元"

    if [[ -f "workflow_state.json" ]]; then
        novel_name=$(grep -o '"name"[[:space:]]*:[[:space:]]*"[^"]*"' workflow_state.json | head -1 | cut -d'"' -f4)
        novel_name=${novel_name:-"星陨纪元"}
    fi

    echo -e "${YELLOW}[1/5] 创建发布目录: $RELEASE_DIR${NC}"
    mkdir -p "$RELEASE_DIR"

    # 生成 changelog
    echo -e "${YELLOW}[2/5] 生成更新日志...${NC}"
    cat > "$RELEASE_DIR/changelog.md" << EOF
# ${novel_name} ${version} 更新日志
...
- TODO: 下一版本计划
EOF
    echo -e "${GREEN}  ✓ 更新日志已创建: $RELEASE_DIR/changelog.md${NC}"

    # 生成 checklist
    echo -e "${YELLOW}[3/5] 生成发布检查清单...${NC}"
...
| 发布执行 | ⏳ | $(date '+%Y-%m-%d') |
EOF
    echo -e "${GREEN}  ✓ 检查清单已创建: $RELEASE_DIR/checklist.md${NC}"

    # 生成版本说明
    echo -e "${YELLOW}[4/5] 生成版本说明...${NC}"
...
## 快速开始
1. 检查 changelog.md 了解本版本变更
2. 查看 checklist.md 确认发布状态
3. 执行最终验证后发布
...
\`\`\`
EOF
    echo -e "${GREEN}  ✓ 版本说明已创建: $RELEASE_DIR/README.md${NC}"
    ...
    echo ""
    echo "下一步:"
    echo "  1. 编辑 changelog.md 填写版本变更"
    echo "  2. 编辑 checklist.md 确认检查项"
    echo "  3. 确认无误后发布"
}
...
do_check_scope() {
    ...
    if [[ -z "$scope" ]]; then
        echo -e "${RED}错误: 请指定检查范围${NC}"
        echo "用法: $0 check <scope> [range]"
        echo "  scope: chapter, volume, all"
        echo "  range: ch001-ch010 (用于chapter)"
        exit 1
    fi

    echo -e "${BLUE}=== 检查发布内容 ===${NC}"
    echo "范围: $scope"
    echo ""

    if [[ "$scope" = "chapter" ]]; then
        if [[ -z "$range" ]]; then
            echo -e "${RED}错误: chapter 模式需要指定范围，例如 ch001-ch010${NC}"
            exit 1
        fi
        echo -e "${YELLOW}检查章节: $range${NC}"
        # 提取章节号
        if [[ "$range" =~ ^ch([0-9]+)-ch([0-9]+)$ ]]; then
            ...
            local chapter_dir="$SCRIPT_DIR/../03_内容仓库/04_正文"
            local missing=0
            for i in $(seq "$start" "$end"); do
                if [[ ! -f "$chapter_dir/ch$(printf '%03d' $i).md" ]]; then
                    echo -e "${RED}  ✗ 缺失: ch$(printf '%03d' $i).md${NC}"
                    missing=$((missing + 1))
                fi
            done
            if [[ $missing -eq 0 ]]; then
                echo -e "${GREEN}  ✓ 所有章节文件存在${NC}"
            else
                echo -e "${RED}  ✗ 共缺失 $missing 个文件${NC}"
            fi
        fi

    elif [[ "$scope" = "volume" ]]; then
        echo -e "${YELLOW}检查卷: $range${NC}"
        local vol_num="$range"
        local vol_dir="$SCRIPT_DIR/../07_汇总仓库/卷正文"
        if [[ -d "$vol_dir" ]]; then
            local vol_file=$(find "$vol_dir" -name "*卷${vol_num}*" -o -name "*第${vol_num}卷*" 2>/dev/null | head -1)
            if [[ -n "$vol_file" ]]; then
                echo -e "${GREEN}  ✓ 卷文件存在: $(basename "$vol_file")${NC}"
            else
                echo -e "${RED}  ✗ 卷文件不存在${NC}"
            fi
        else
            echo -e "${YELLOW}  ⚠ 卷正文目录不存在${NC}"
        fi

    elif [[ "$scope" = "all" ]]; then
        echo -e "${YELLOW}检查全部内容${NC}"
        # 检查章节文件
        local chapter_count=$(ls "$SCRIPT_DIR/../03_内容仓库/04_正文"/ch*.md 2>/dev/null | wc -l)
        echo "  章节文件: $chapter_count 个"
        # 检查审核报告
        local audit_count=$(ls "$SCRIPT_DIR/../06_意见仓库/04_正文_审核"/*_审核.md 2>/dev/null | wc -l)
        echo "  审核报告: $audit_count 个"
        # 检查汇总
        local summary_count=$(ls "$SCRIPT_DIR/../07_汇总仓库/全文正文"/*.md 2>/dev/null | wc -l)
        echo "  全文汇总: $summary_count 个"
    fi
...
do_list_enhanced() {
    ...
    if [[ -d "$releases_dir" ]]; then
        echo -e "${YELLOW}发布记录:${NC}"
        find "$releases_dir" -maxdepth 1 -type d -name "v*" 2>/dev/null | sort -r | while read dir; do
            local version=$(basename "$dir")
            local date=$(echo "$version" | grep -oE '[0-9]{4}-[0-9]{2}-[0-9]{2}' || echo "未知")
            local changelog="$dir/changelog.md"
            local status="未知"
            if [[ -f "$changelog" ]]; then
                if grep -q "状态.*正式发布" "$changelog"; then
                    status="正式发布"
                elif grep -q "状态.*预发布" "$changelog"; then
                    status="预发布"
                fi
            fi
            echo "  - $version ($status)"
...
do_diff_versions() {
    ...
    if [[ -z "$from" || -z "$to" ]]; then
        echo -e "${RED}错误: 请指定两个版本号，例如: diff-versions v4.0 v5.0${NC}"
        exit 1
    fi

    echo -e "${BLUE}=== 版本对比: $from → $to ===${NC}"
    echo ""

    local releases_dir="$SCRIPT_DIR/../releases"
    local from_dir=$(find "$releases_dir" -maxdepth 1 -type d -name "*${from}*" 2>/dev/null | head -1)
    local to_dir=$(find "$releases_dir" -maxdepth 1 -type d -name "*${to}*" 2>/dev/null | head -1)

    echo "版本1目录: ${from_dir:-（未找到）}"
    echo "版本2目录: ${to_dir:-（未找到）}"
    echo ""

    if [[ -z "$from_dir" || -z "$to_dir" ]]; then
        echo -e "${RED}错误: 无法找到两个版本${NC}"
        exit 1
    fi

    # 对比 changelog
    echo -e "${YELLOW}更新日志对比:${NC}"
    local from_log="$from_dir/changelog.md"
    local to_log="$to_dir/changelog.md"

    if [[ -f "$from_log" && -f "$to_log" ]]; then
        echo "  $from ($(wc -l < "$from_log") 行)"
        echo "  $to ($(wc -l < "$to_log") 行)"
        echo ""
        echo -e "${YELLOW}内容差异:${NC}"
        diff -u "$from_log" "$to_log" | head -30 || true
    else
        echo "  （无法对比）"
    fi
}
...
case "${1:-}" in
    check)
        # 增强版 check 支持多种模式
        if [[ "$2" == "chapter" || "$2" == "volume" || "$2" == "all" ]]; then
            do_check_scope "$2" "$3"
        else
            # 原有 check 命令
            do_check "$2"
