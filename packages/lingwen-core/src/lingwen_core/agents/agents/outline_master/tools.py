...
class OutlineMasterTools:
    ...
    def generate_outline(self, settings: Dict, requirements: Dict) -> Dict:
        ...
        outline = {
            ...
            "chapters": self._generate_chapters(settings, requirements.get("total_chapters", 360)),
        }
        self.schema.validate(outline)
        return outline

    def generate_chapter_outline(
        self, chapter_num: int, events: List[str], foreshadow: Optional[List[str]] = None
    ) -> Dict:
        """生成章大纲"""
        return {
            "num": chapter_num,
    ...
    def _generate_chapters(self, settings: Dict, total: int) -> List[Dict]:
        ...
        for i in range(1, total + 1):
            chapters.append({"num": i, "title": f"第{i}章", "events": [], "word_count_target": 2500})
        return chapters
