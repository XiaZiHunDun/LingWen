...
def _load_patterns_from_yaml() -> Tuple[List[tuple], List[tuple]]:
    ...
    template_patterns = [
        (
            r"他[说问道喊叫笑叹谓称著显示露出透露出冒][^。！？]{0,15}[。！？]",
            "模板_他说道",
            ['减少"他说道"的使用，用动作和表情替代', "改为：他抬起下巴，目光冷冽", "改为：嘴角勾起一抹笑"],
        ),
        (
            r"她[说问道喊叫笑叹谓称著显示露出透露出冒][^。！？]{0,15}[。！？]",
            "模板_她说道",
            [
                '减少"她说道"的使用，用动作和表情替代',
                "改为：她轻抚发丝，声音低沉",
        ...
        (
            r"只见([^，]。*)",
            "模板_只见",
            ['"只见"过于套路化', "替换：视野中突现、眼前浮现、一道身影落下", "示例：视野中突现一道身影"],
        ),
        (
            r"突然([^，]。*)",
            "模板_突然",
            ['"突然"过于平淡', "替换：倏忽、霎时、刹那、一瞬间、蓦然", "示例：空气中霎时弥漫起一股寒意"],
        ),
        (
            r"然而([^，]。*)",
            "模板_然而",
            ['过多"然而"削弱转折力度', "替换：但、只是、可惜、无奈、却、偏", "示例：可惜天不遂人愿"],
        ),
        (
            r"但是([^，]。*)",
            "模板_但是",
            ['过多"但是"削弱转折力度', "替换：只是、可、可惜、无奈、却", "示例：只是这代价太过沉重"],
        ),
        (
            r"因此([^，]。*)",
            "模板_因此",
            ['"因此"过于书面化', "替换：于是、这就、这才、于是乎", "示例：于是众人各自散去"],
        ),
        (
            r"随着([^，]。*)",
            "模板_随着",
            ['"随着"过于套路化', "替换：此后、此后不久、就在此时、就在那刻", "示例：就在此时，异变突生"],
        ),
        (r"如果([^，]。*)", "模板_如果", ["虚拟语气过多使用", '减少"如果...就"结构，用事实陈述替代']),
        (
            r"于是([^，]。*)",
            "模板_于是",
            ['"于是"使用过度', "替换：接着、随后、然后、此时、下一秒", "示例：下一秒，他消失在原地"],
        ),
        (
            r"就在这时([^，]。*)",
            "模板_就在此时",
            ["时间标记过于套路", "替换：恰在此时、偏在这时、正此际", "示例：恰在此时，一道闪电划破夜空"],
        ),
        (
            r"不一会儿([^，]。*)",
            "模板_不一会儿",
            ["时间过渡过于简单", "替换：片刻后、少顷、须臾之间、转瞬", "示例：少顷，他睁开双眼"],
        ),
        (
            r"很快([^，]。*)",
            "模板_很快",
            ["时间过渡过于模糊", "替换：不多时、半晌、一炷香后", "示例：半晌，他才回过神来"],
        ),
        (
            r"看到这一幕([^，]。*)",
            "模板_看到这一幕",
            ["视角转换过于生硬", "替换：目击此景、见状、这一幕映入眼帘", "示例：这一幕映入眼帘，他心头一沉"],
        ),
        (
            r"听到这话([^，]。*)",
            "模板_听到这话",
            ["对话引入过于生硬", "替换：话音入耳、闻及此言、此言入心", "示例：话音入耳，他面色微变"],
        ),
    ]
        ...
        try:
            with open(diverse_path, encoding="utf-8") as f:
                ...
                if data and "diverse_patterns" in data:
                    diverse_patterns = [
                        (p["pattern"], p["name"], p["label"]) for p in data["diverse_patterns"]
                    ]
        ...
        try:
            with open(template_path, encoding="utf-8") as f:
                ...
                if data and "template_patterns" in data:
                    template_patterns = [
                        (p["pattern"], p["name"], p.get("suggestions", [])) for p in data["template_patterns"]
                    ]
        except Exception as e:
