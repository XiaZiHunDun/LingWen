...
class CrossChapterLogicChecker(BaseChecker):
    ...
    def _detect_state_contradictions(
        self, chapter_num: int, content: str, entity_states: Dict[int, Dict[str, EntityState]]
    ) -> List[Issue]:
        """
        检测状态矛盾
        ...
        for ch in range(1, chapter_num):
            if ch in entity_states:
                for name, state in entity_states[ch].items():
                    if (
                        name not in previous_states
                        or state.chapter_last_mentioned > previous_states[name].chapter_last_mentioned
                    ):
        ...
        for line_num, line in enumerate(lines, 1):
            ...
            if re.search(r"没有埋葬|(?<!没有)埋葬.*(?:#|尸体|遗骸)", line):
                if "老人" in line:
                    ...
                    has_departure_in_prev = (
                        "老人" in previous_states and previous_states["老人"].state == "left"
                    )
    ...
    def check_with_full_scan(self, chapters: List[Tuple[int, str]]) -> List[Issue]:
        """
        全量扫描模式（批量检测时调用）
