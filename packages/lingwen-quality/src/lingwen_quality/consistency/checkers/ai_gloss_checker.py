...
class AIGlossChecker(BaseChecker):
    ...
    def _default_rules(self) -> Dict[str, Any]:
        ...
        return {
            "patterns": {
                ...
                "ai_phrases": ["作为一个AI", "我作为一个语言模型", "从技术角度来说", "从理论上来讲"],
            },
            "severity": {"P3_count_threshold": 3, "density_threshold": 0.05},
        }
    ...
    def check(
        ...
        # 检查过度总结
        issues.extend(
            self._check_over_summary(chapter_content, chapter_num, patterns.get("over_summary", []))
        )

        # 检查机械过渡
        issues.extend(
            self._check_mechanical_transition(
                chapter_content, chapter_num, patterns.get("mechanical_transition", [])
            )
        )

        # 检查格式化表达
        issues.extend(self._check_formatting(chapter_content, chapter_num, patterns.get("formatting", [])))

        # 检查AI特有表达
        issues.extend(self._check_ai_phrases(chapter_content, chapter_num, patterns.get("ai_phrases", [])))

        return issues

    def _check_over_summary(self, content: str, chapter_num: int, keywords: List[str]) -> List[Issue]:
        """检查过度总结类表达"""
        issues = []
    ...
    def _check_mechanical_transition(
        self, content: str, chapter_num: int, keywords: List[str]
    ) -> List[Issue]:
        """检查机械过渡"""
        issues = []
        severity_rules = self.rules.get("severity", {})
    ...
    def _check_formatting(self, content: str, chapter_num: int, keywords: List[str]) -> List[Issue]:
        """检查格式化表达"""
        issues = []
    ...
    def _check_ai_phrases(self, content: str, chapter_num: int, keywords: List[str]) -> List[Issue]:
        """检查AI特有短语"""
        issues = []
