...
Phase 51 P3-ARCHDEBT: MERGED from infra/prose_calibration_overrides.py (174 LOC, 9 funcs).
Reuses package-level `_FACTORY_ROOT` from service.py (parents[4]) so the
yaml config path stays at repo_root/config/prose_calibration_overrides.yaml.
"""
...
def load_all_calibration_overrides(
    *,
    ...
    log_overrides = parse_markdown_log_overrides(
        log_path or _FACTORY_ROOT / "docs" / "prose-calibration-log.md",
    )
