...
TRUE LEAF — only stdlib (functools/pathlib/typing) + PyYAML.

Architecture note: _FACTORY_ROOT resolves via parents[4] from the new
package location (packages/lingwen-prose-calibration/src/lingwen_prose_calibration/service.py)
to the repo root. This is the same pattern as Phase 40a C1.5 factory_root fixup.
"""
...
# Phase 44: parents[4] from new location → repo root
# (was parents[1] from infra/prose_calibration.py)
