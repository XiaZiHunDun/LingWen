"""LLMService — concrete multi-provider LLM service with runtime failover.

Phase 43 P3-ARCHDEBT: relocated from infra/llm_service.py (service.py portion).
Public class; singleton via LLMService.get().

Architecture note: business code MUST NOT import LLMService directly (DP-02).
Use LLMServiceAdapter from lingwen_llm.port_adapter instead. The factory
registration that wires up LLMServiceAdapter() default behavior happens at
module load of lingwen_llm_service/__init__.py (see factory.py).
"""
...
class LLMService:
    ...
    def __init__(self):
        # 延迟导入 lingwen_llm.providers 以避免 lingwen_llm_service/__init__.py
        # 加载时触发循环依赖 (Phase 17.5: ai_service 迁出后)。
