...
> **下一协作**: Track A 已自服务完成 P1-SHIM + P2-DTOMIGR + P2-DRAWER + P2-INSIGHT（前端 P2 系列收尾）
...
| **git main** | master（最近 `d3347d3b`）|
| **当前阶段** | A=前端 P1-SHIM；B=后端 P2-QUEUE ✅ → P2-RESTART（进行中） |
...
| ID | 任务 | 进度 | 认领人 | 状态 |
|----|------|------|--------|------|
| P2-RESTART | auto-restart on failure（后端） | 后端 B 已认领 | 后端 B | 🔄 进行中 |

> 详见 [BACKLOG.md](BACKLOG.md)：P1-SHIM（✅）/ P2-DTOMIGR（✅）/ P2-DRAWER（✅）/ P2-INSIGHT（✅）/ P2-QUEUE（✅）；后端 B 当前进行 P2-RESTART。
