...
| 项目 | 状态 |
|------|------|
| **版本** | v39.0（Phase 40a — P3-ARCHDEBT studio_registry 5/5a 闭环：infra/studio_registry.py → packages/lingwen-studio-registry/；infra/studio_registry.py 留 shim，Phase 40b 删；invariant #55 NEW） |
| **git main** | master `1c473405`（v39.0 ff-merge 完成，8 commit `6ba28672..1c473405`） |
...
| **P3-ARCHDEBT 5/5b** | **`tests/` root 仍有 ~33 处引用 `infra.studio_registry`（24 files: 21 `from` + 4 `import as` + 7 `monkeypatch.setattr` + 1 doc）。生产代码已 canonical (Phase 40a)；shim 留着只为 tests/。Phase 40b: 应用 `\1` whitespace sed 到 tests/ → DELETE `infra/studio_registry.py` shim → DELETE `infra/studio/__init__.py:2` wildcard → +33 回归 guard tests。Plan for 4-5 atomic commits (C0 spec+plan / C1 test-migration / C2 shim-delete / C3 guards / C4 invariant-update)。机械性迁移，可 1 worktree + 多 commits 完成** | Phase 40a (P3-ARCHDEBT 5/5a) | 📋 Phase 40b 待启动 |
