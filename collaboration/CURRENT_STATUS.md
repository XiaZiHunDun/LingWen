...
> **下一协作**: v30.0 TACKLE-14-FAILURES 已闭环：14 → 0 failed（-14），tests/agent_system 480 passed / 0 failed / 20 skipped；tests/got 156 / lingwen-core 68 / ruff clean 不变。**2 真 prod 改动**（mc_writing.py 韧性契约 + cost_persistence.py record_at helper）+ 5 test 文件调整。剩余 carryover：P2-ARCHDEBT（infra.got 迁移 + chapter_golden_path 反向 import + 5 薄代理 → OrchestratorProxyMixin + PHASE-COMPAT shim 删）/ Prod preview regression（accepted debt 不动）
